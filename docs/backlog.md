...
**CSS:**
1. **Toggle switch CSS duplicated** — identical in `AdminPage.css` and `ProfilePage.css`. Move to `utilities.css`.
2. **Responsive grid CSS re-declared per page** — `utilities.css` already has `.grid-responsive-*` classes but `StorePage.css`, `RunPage.css`, `GroupPage.css` re-declare the same grid pattern.

**Docker & Config:**
3. **Base compose has dev-only backend volume mount** — `./backend:/app` in `docker-compose.yml` should only be in `docker-compose.dev.yml`.
4. **Backend missing `depends_on: redis`** — backend uses Redis for sessions but doesn't wait for it to be healthy.
5. **Stale files to remove** — `UNIFIED_CONFIG.md` (documents non-existent config), `MIGRATION_USERNAME.md` (completed migration), `frontend/.env.example` (redundant).

**Backend:**
6. **Manual UUID parsing in ShoppingService** — does `try: UUID(id) / except ValueError` inline ~7 times instead of using existing `validate_uuid()` utility.
7. **ReassignmentService duplicate validation** — `_validate_accept_request` and `_validate_decline_request` are near-identical, merge into one method.
8. **Shared helpers duplicated across services** — `_get_store_name`, `_is_leader_or_helper`, `_get_user_participation` each exist in 2+ services. Move to `BaseService`.

**Frontend:**
9. **NewProductPopup and NewRunPopup manually fetch stores** — both implement `fetchStores()` in useEffect instead of using existing `useStores` React Query hook.
10. **PricePopup, PurchasePopup, EditCommentPopup bypass BaseModal** — hand-roll modal overlay + stopPropagation instead of using `BaseModal` like all other popups.
