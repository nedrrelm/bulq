...
NOTE: These tests are for an old monolithic repository pattern that no longer exists.
The current architecture uses domain-specific repositories (UserRepository, GroupRepository, etc.).
These tests are skipped pending migration to the new architecture.
"""
...
pytestmark = pytest.mark.skip(
    reason='Tests for deprecated monolithic repository pattern - needs migration to domain-specific repositories'
)
...
class TestMemoryRepository:
    ...
    def repo(self):
        ...
        pytest.skip('Monolithic repository pattern no longer exists')
    ...
    def test_get_user_by_email_seed_data(self, repo):
        """Test email lookup works with seed data"""
    ...
    def test_add_user_to_group(self, repo):
        ...
        creator = repo.create_user(
            name='Creator', email='creator@example.com', password_hash='hash'
        )
    ...
    def test_get_group_members(self, repo):
        ...
        test_group = next(
            (g for g in repo.get_user_groups(alice) if g.name == 'Test Friends'), None
        )
    ...
    def test_create_participation(self, repo):
        ...
        user = repo.create_user(
            name='User', email='newparticipant@example.com', password_hash='hash'
        )
        ...
        participation = repo.create_participation(user_id=user.id, run_id=run.id, is_leader=False)
    ...
    def test_create_shopping_list_item(self, repo):
        ...
        item = repo.create_shopping_list_item(
            run_id=run.id, product_id=products[0].id, requested_quantity=10
        )
...
@pytest.mark.skip('DatabaseRepository not implemented')
class TestDatabaseRepository:
    ...
    def repo(self, db_session):
        ...
        pytest.skip('DatabaseRepository not implemented')
