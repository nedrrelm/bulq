...
class TestCreateUser:
    ...
    async def test_create_user_with_required_fields(self, repo, sample_user_data):
    ...
    async def test_created_user_has_uuid(self, repo, sample_user_data):
    ...
    async def test_created_user_is_stored_and_retrievable(self, repo, sample_user_data):
    ...
    async def test_created_user_has_default_values(self, repo, sample_user_data):
    ...
    async def test_password_hash_is_stored(self, repo, sample_user_data):
    ...
    async def test_creating_multiple_users_different_ids(self, repo):
        """Test creating multiple users generates different IDs."""
        user1 = await repo.create_user('User One', 'user1', 'hash1')
        user2 = await repo.create_user('User Two', 'user2', 'hash2')

        assert user1.id != user2.id
        assert user1.username != user2.username

    async def test_create_user_username_index_updated(self, repo, sample_user_data):
        """Test user is indexed by username."""
        user = await repo.create_user(**sample_user_data)

        retrieved = await repo.get_user_by_username(sample_user_data['username'])
        assert retrieved is not None
        assert retrieved.id == user.id
...
class TestGetUserById:
    ...
    async def test_get_existing_user_by_id(self, repo, sample_user_data):
    ...
    async def test_get_nonexistent_user_returns_none(self, repo):
        ...
        result = await repo.get_user_by_id(fake_id)
        assert result is None

    async def test_get_user_by_id_after_creation(self, repo, sample_user_data):
    ...
    async def test_get_user_by_id_after_update(self, repo, sample_user_data):
        """Test getting user after update returns updated data."""
        user = await repo.create_user(**sample_user_data)
        await repo.update_user(user.id, name='Updated Name')
...
class TestGetUserByUsername:
    ...
    async def test_get_user_by_username(self, repo, sample_user_data):
    ...
    async def test_get_nonexistent_username_returns_none(self, repo):
        """Test non-existent username returns None."""
        result = await repo.get_user_by_username('nonexistent')
        assert result is None

    async def test_username_case_sensitive(self, repo):
        """Test username lookup is case-sensitive."""
        await repo.create_user('User', 'TestUser', 'hash')

        # Exact match should work
        assert await repo.get_user_by_username('TestUser') is not None

        # Different case should not match
        assert await repo.get_user_by_username('testuser') is None
        assert await repo.get_user_by_username('TESTUSER') is None

    async def test_get_user_by_username_after_update(self, repo, sample_user_data):
        """Test getting user by new username after update."""
        user = await repo.create_user(**sample_user_data)
        new_username = 'newusername'

        await repo.update_user(user.id, username=new_username)

        # Old username should not work
        assert await repo.get_user_by_username(sample_user_data['username']) is None

        # New username should work
        retrieved = await repo.get_user_by_username(new_username)
        assert retrieved is not None
        assert retrieved.id == user.id
...
class TestUpdateUser:
    ...
    async def test_update_user_name(self, repo, sample_user_data):
        """Test updating user name."""
        user = await repo.create_user(**sample_user_data)
        new_name = 'Jane Smith'

        updated = await repo.update_user(user.id, name=new_name)

        assert updated is not None
        assert updated.name == new_name
        assert updated.id == user.id

    async def test_update_user_password_hash(self, repo, sample_user_data):
    ...
    async def test_update_user_is_admin(self, repo, sample_user_data):
        """Test updating user is_admin flag."""
        user = await repo.create_user(**sample_user_data)

        updated = await repo.update_user(user.id, is_admin=True)

        assert updated.is_admin is True

    async def test_update_user_verified(self, repo, sample_user_data):
        """Test updating user verified flag."""
        user = await repo.create_user(**sample_user_data)

        updated = await repo.update_user(user.id, verified=True)
    ...
    async def test_update_nonexistent_user_returns_none(self, repo):
        ...
        result = await repo.update_user(fake_id, name='New Name')
        assert result is None

    async def test_update_partial_fields(self, repo, sample_user_data):
        """Test partial updates (only some fields)."""
        user = await repo.create_user(**sample_user_data)
        original_username = user.username

        updated = await repo.update_user(user.id, name='New Name')

        assert updated.name == 'New Name'
        assert updated.username == original_username  # Unchanged

    async def test_update_multiple_fields(self, repo, sample_user_data):
        """Test updating multiple fields at once."""
        user = await repo.create_user(**sample_user_data)

        updated = await repo.update_user(user.id, name='New Name', is_admin=True, verified=True)

        assert updated.name == 'New Name'
        assert updated.is_admin is True
        assert updated.verified is True

    async def test_update_username_updates_index(self, repo, sample_user_data):
    ...
    async def test_updated_fields_are_persisted(self, repo, sample_user_data):
        """Test updated fields are persisted."""
        user = await repo.create_user(**sample_user_data)
        await repo.update_user(user.id, name='Updated Name', is_admin=True)
...
class TestDeleteUser:
    ...
    async def test_user_not_retrievable_after_deletion(self, repo, sample_user_data):
    ...
    async def test_delete_nonexistent_user_returns_false(self, repo):
        ...
        result = await repo.delete_user(fake_id)
        assert result is False

    async def test_deleting_twice_returns_false_second_time(self, repo, sample_user_data):
        """Test deleting twice returns False second time."""
        user = await repo.create_user(**sample_user_data)

        first_delete = await repo.delete_user(user.id)
        second_delete = await repo.delete_user(user.id)

        assert first_delete is True
        assert second_delete is False

    async def test_delete_by_id(self, repo, sample_user_data):
        """Test delete by ID."""
        user = await repo.create_user(**sample_user_data)
        user_id = user.id
...
class TestGetAllUsers:
    ...
    async def test_list_all_after_creating_multiple_users(self, repo):
        """Test list_all after creating multiple users."""
        user1 = await repo.create_user('User 1', 'user1', 'hash1')
        user2 = await repo.create_user('User 2', 'user2', 'hash2')
        user3 = await repo.create_user('User 3', 'user3', 'hash3')
    ...
    async def test_list_all_returns_all_users(self, repo):
        """Test list_all returns all users."""
        created_users = []
        for i in range(5):
            user = await repo.create_user(f'User {i}', f'user{i}', f'hash{i}')
    ...
    async def test_list_all_includes_all_user_fields(self, repo, sample_user_data):
    ...
    async def test_list_all_count_matches_created(self, repo):
        ...
        for i in range(count):
            await repo.create_user(f'User {i}', f'user{i}', f'hash{i}')
    ...
    async def test_list_all_after_deletion(self, repo):
        ...
        assert len(await repo.get_all_users()) == 3
...
class TestGetUserGroups:
    ...
    async def test_get_user_groups_empty(self, repo, storage, sample_user_data):
    ...
    async def test_get_user_groups_single_group(self, repo, storage, sample_user_data):
    ...
    async def test_get_user_groups_multiple_groups(self, repo, storage, sample_user_data):
...
class TestGetUserStats:
    ...
    async def test_get_user_stats_empty(self, repo, sample_user_data):
    ...
    async def test_get_user_stats_with_groups(self, repo, storage, sample_user_data):
    ...
    async def test_get_user_stats_with_participations(self, repo, storage, sample_user_data):
...
class TestBulkUpdateRunParticipations:
    ...
    async def test_bulk_update_run_participations(self, repo, storage):
        """Test bulk updating run participations."""
        old_user = await repo.create_user('Old User', 'olduser', 'hash1')
        new_user = await repo.create_user('New User', 'newuser', 'hash2')
        ...
        count = await repo.bulk_update_run_participations(old_user.id, new_user.id)

        assert count == 2
        assert storage.participations[part1_id].user_id == new_user.id
        assert storage.participations[part2_id].user_id == new_user.id

    async def test_bulk_update_run_participations_no_matches(self, repo, storage):
        """Test bulk update with no matching participations."""
        user1 = await repo.create_user('User 1', 'user1', 'hash1')
        user2 = await repo.create_user('User 2', 'user2', 'hash2')

        count = await repo.bulk_update_run_participations(user1.id, user2.id)
...
class TestBulkUpdateGroupCreator:
    ...
    async def test_bulk_update_group_creator(self, repo, storage):
        ...
        count = await repo.bulk_update_group_creator(old_user.id, new_user.id)
...
class TestCheckOverlappingRunParticipations:
    ...
    async def test_no_overlapping_participations(self, repo, storage):
        """Test users with no overlapping run participations."""
        user1 = await repo.create_user('User 1', 'user1', 'hash1')
        user2 = await repo.create_user('User 2', 'user2', 'hash2')

        overlaps = await repo.check_overlapping_run_participations(user1.id, user2.id)

        assert overlaps == []

    async def test_overlapping_participations(self, repo, storage):
...
class TestEdgeCases:
    ...
    async def test_very_long_username(self, repo):
        """Test with very long username."""
        long_username = 'a' * 1000
        await repo.create_user('User', long_username, 'hash')

        retrieved = await repo.get_user_by_username(long_username)
        assert retrieved is not None
    ...
    async def test_special_characters_in_name(self, repo):
        ...
        user = await repo.create_user(special_name, 'obrien', 'hash')
    ...
    async def test_unicode_characters(self, repo):
        ...
        user = await repo.create_user(unicode_name, unicode_username, 'hash')

        assert user.name == unicode_name
        assert user.username == unicode_username
        retrieved = await repo.get_user_by_username(unicode_username)
        assert retrieved is not None
...
class TestDataIntegrity:
    ...
    async def test_user_object_has_expected_fields(self, repo, sample_user_data):
    ...
    async def test_user_object_is_not_none(self, repo, sample_user_data):
    ...
    async def test_multiple_repositories_share_storage(self, storage):
        ...
        user = await repo1.create_user('User', 'username', 'hash')

        # Both repositories should see the same user
        assert await repo2.get_user_by_id(user.id) is not None
        assert await repo2.get_user_by_username('username') is not None


class TestVerifyPassword:
    """Test verify_password() method."""

    async def test_verify_password_correct(self, repo):
        ...
        result = await repo.verify_password(password, hashed)
        assert result is True

    async def test_verify_password_incorrect(self, repo):
        ...
        result = await repo.verify_password('wrongpassword', hashed)
        assert result is False
...
class TestTransferGroupAdminStatus:
    ...
    async def test_transfer_group_admin_status(self, repo, storage):
        """Test transferring group admin status."""
        old_admin = await repo.create_user('Old Admin', 'oldadmin', 'hash1')
        new_admin = await repo.create_user('New Admin', 'newadmin', 'hash2')
        ...
        count = await repo.transfer_group_admin_status(old_admin.id, new_admin.id)

        assert count == 1
        assert storage.group_admin_status[(group_id, new_admin.id)] is True

    async def test_transfer_group_admin_status_not_member(self, repo, storage):
