...
class TestCreateStore:
    ...
    async def test_create_store_with_required_fields(self, repo, sample_store_data):
        """Test creating store with required fields."""
        store = await repo.create_store(**sample_store_data)

        assert store is not None
        assert store.name == sample_store_data['name']

    async def test_create_store_with_name_only(self, repo):
        """Test creating store with name only."""
        store = await repo.create_store(name='Simple Store')

        assert store is not None
        assert store.name == 'Simple Store'

    async def test_created_store_has_uuid(self, repo, sample_store_data):
    ...
    async def test_created_store_is_stored_and_retrievable(self, repo, sample_store_data):
        """Test created store is stored and retrievable."""
        store = await repo.create_store(**sample_store_data)

        retrieved = await repo.get_store_by_id(store.id)
        assert retrieved is not None
        assert retrieved.id == store.id
        assert retrieved.name == store.name

    async def test_created_store_has_default_values(self, repo, sample_store_data):
        """Test created store has default values (verified=False)."""
        store = await repo.create_store(**sample_store_data)

        assert store.verified is False

    async def test_creating_multiple_stores_different_ids(self, repo):
        """Test creating multiple stores generates different IDs."""
        store1 = await repo.create_store('Store One')
        store2 = await repo.create_store('Store Two')

        assert store1.id != store2.id
        assert store1.name != store2.name

    async def test_create_store_minimal(self, repo):
        """Test creating store with minimal data."""
        store = await repo.create_store(name='Minimal Store')
...
class TestGetStoreById:
    ...
    async def test_get_existing_store_by_id(self, repo, sample_store_data):
    ...
    async def test_get_nonexistent_store_returns_none(self, repo):
        """Test getting non-existent store returns None."""
        fake_id = uuid4()

        result = await repo.get_store_by_id(fake_id)
        assert result is None

    async def test_get_store_by_id_after_creation(self, repo, sample_store_data):
    ...
    async def test_get_store_by_id_after_update(self, repo, sample_store_data):
        """Test getting store after update returns updated data."""
        store = await repo.create_store(**sample_store_data)
        await repo.update_store(store.id, name='Updated Name')
...
class TestUpdateStore:
    ...
    async def test_update_store_name(self, repo, sample_store_data):
        """Test updating store name."""
        store = await repo.create_store(**sample_store_data)
        new_name = 'TestMart Uptown'

        updated = await repo.update_store(store.id, name=new_name)

        assert updated is not None
        assert updated.name == new_name
        assert updated.id == store.id

    async def test_update_store_address(self, repo, sample_store_data, storage, sample_user):
        ...
        updated = await repo.update_store(store.id, address='456 New Ave')

        assert updated.address == '456 New Ave'

    async def test_update_store_chain(self, repo, sample_store_data, storage, sample_user):
        ...
        updated = await repo.update_store(store.id, chain='NewMart')

        assert updated.chain == 'NewMart'

    async def test_update_store_opening_hours(self, repo, sample_store_data, storage, sample_user):
        ...
        updated = await repo.update_store(store.id, opening_hours=opening_hours)

        assert updated.opening_hours == opening_hours

    async def test_update_store_verified(self, repo, sample_store_data, admin_user, storage):
    ...
    async def test_update_nonexistent_store_returns_none(self, repo):
        """Test updating non-existent store returns None."""
        fake_id = uuid4()

        result = await repo.update_store(fake_id, name='New Name')
        assert result is None

    async def test_update_partial_fields(self, repo, sample_store_data, storage, sample_user):
        ...
        updated = await repo.update_store(store.id, name='New Name')

        assert updated.name == 'New Name'
        assert updated.chain == original_chain  # Unchanged

    async def test_update_multiple_fields(self, repo, sample_store_data, storage, sample_user):
    ...
    async def test_updated_fields_are_persisted(self, repo, sample_store_data):
        """Test updated fields are persisted."""
        store = await repo.create_store(**sample_store_data)
        await repo.update_store(store.id, name='Updated Name', verified=True)
...
class TestDeleteStore:
    ...
    async def test_delete_existing_store(self, repo, sample_store_data):
        """Test deleting existing store."""
        store = await repo.create_store(**sample_store_data)

        result = await repo.delete_store(store.id)

        assert result is True

    async def test_store_not_retrievable_after_deletion(self, repo, sample_store_data):
        """Test store not retrievable after deletion."""
        store = await repo.create_store(**sample_store_data)
        await repo.delete_store(store.id)

        retrieved = await repo.get_store_by_id(store.id)
        assert retrieved is None

    async def test_delete_nonexistent_store_returns_false(self, repo):
        """Test deleting non-existent store returns False."""
        fake_id = uuid4()

        result = await repo.delete_store(fake_id)
        assert result is False

    async def test_deleting_twice_returns_false_second_time(self, repo, sample_store_data):
        """Test deleting twice returns False second time."""
        store = await repo.create_store(**sample_store_data)

        first_delete = await repo.delete_store(store.id)
        second_delete = await repo.delete_store(store.id)
...
class TestGetAllStores:
    ...
    async def test_list_all_after_creating_multiple_stores(self, repo):
        """Test list_all after creating multiple stores."""
        store1 = await repo.create_store('Store 1')
        store2 = await repo.create_store('Store 2')
        store3 = await repo.create_store('Store 3')
    ...
    async def test_list_all_returns_all_stores(self, repo):
        """Test list_all returns all stores."""
        created_stores = []
        for i in range(5):
            store = await repo.create_store(f'Store {i}')
    ...
    async def test_list_all_includes_all_store_fields(self, repo, sample_store_data):
    ...
    async def test_list_all_count_matches_created(self, repo):
        ...
        for i in range(count):
            await repo.create_store(f'Store {i}')
    ...
    async def test_list_all_after_deletion(self, repo):
        ...
        assert len(await repo.get_all_stores()) == 3

        await repo.delete_store(store2.id)
    ...
    async def test_list_all_sorted_by_name(self, repo):
        """Test list_all returns stores sorted by name."""
        await repo.create_store('Zebra Store')
        await repo.create_store('Alpha Store')
        await repo.create_store('Beta Store')
    ...
    async def test_list_all_with_pagination(self, repo):
        ...
        for i in range(10):
            await repo.create_store(f'Store {i:02d}')

        # Get first 3 stores
        stores = await repo.get_all_stores(limit=3, offset=0)
        assert len(stores) == 3

        # Get next 3 stores
        stores = await repo.get_all_stores(limit=3, offset=3)
        assert len(stores) == 3

        # Get last 4 stores
        stores = await repo.get_all_stores(limit=10, offset=6)
        assert len(stores) == 4
...
class TestSearchStores:
    ...
    async def test_search_by_name_exact_match(self, repo):
        ...
        store = await repo.create_store('Downtown Costco')
        await repo.create_store('Uptown Walmart')

        results = await repo.search_stores('Downtown Costco')
    ...
    async def test_search_by_name_partial_match(self, repo):
        ...
        store1 = await repo.create_store('Downtown Costco')
        store2 = await repo.create_store('Uptown Costco')
        await repo.create_store('Walmart')

        results = await repo.search_stores('Costco')
    ...
    async def test_search_with_no_matches(self, repo):
        ...
        await repo.create_store('Downtown Costco')

        results = await repo.search_stores('Walmart')

        assert results == []

    async def test_search_empty_query(self, repo):
        """Test search with empty query returns all stores."""
        await repo.create_store('Store 1')
        await repo.create_store('Store 2')

        results = await repo.search_stores('')

        assert len(results) == 2

    async def test_search_in_empty_repository(self, repo):
        """Test search in empty repository."""
        results = await repo.search_stores('anything')

        assert results == []

    async def test_search_by_lowercase(self, repo):
        """Test search with lowercase query matches uppercase store."""
        store = await repo.create_store('UPPERCASE STORE')

        results = await repo.search_stores('uppercase')
    ...
    async def test_search_by_uppercase(self, repo):
        ...
        store = await repo.create_store('lowercase store')

        results = await repo.search_stores('LOWERCASE')
...
class TestGetProductsByStore:
    ...
    async def test_get_products_by_store(self, repo, storage, sample_user):
        """Test getting products available at a store."""
        store = await repo.create_store('Test Store')
        ...
        products = await repo.get_products_by_store_from_availabilities(store.id)
    ...
    async def test_get_products_by_store_empty(self, repo):
        """Test getting products for store with no availabilities."""
        store = await repo.create_store('Empty Store')

        products = await repo.get_products_by_store_from_availabilities(store.id)

        assert products == []

    async def test_get_products_by_store_unique_products(self, repo, storage, sample_user):
...
class TestGetActiveRunsByStoreForUser:
    ...
    async def test_get_active_runs_by_store_for_user(self, repo, storage, sample_user):
        ...
        runs = await repo.get_active_runs_by_store_for_user(store.id, sample_user.id)

        assert len(runs) == 1
        assert runs[0].id == run.id

    async def test_get_active_runs_multiple_states(self, repo, storage, sample_user):
    ...
    async def test_get_active_runs_excludes_completed(self, repo, storage, sample_user):
    ...
    async def test_get_active_runs_user_not_in_group(self, repo, storage, sample_user):
    ...
    async def test_get_active_runs_different_store(self, repo, storage, sample_user):
        ...
        # Query for store1
        runs = await repo.get_active_runs_by_store_for_user(store1.id, sample_user.id)
...
class TestBulkUpdateOperations:
    ...
    async def test_bulk_update_runs(self, repo, storage, sample_user):
        """Test bulk updating runs."""
        old_store = await repo.create_store('Old Store')
        new_store = await repo.create_store('New Store')
        ...
        count = await repo.bulk_update_runs(old_store.id, new_store.id)

        assert count == 2
        assert storage.runs[run1.id].store_id == new_store.id
        assert storage.runs[run2.id].store_id == new_store.id

    async def test_bulk_update_runs_no_matches(self, repo):
        """Test bulk update with no matching runs."""
        store1 = await repo.create_store('Store 1')
        store2 = await repo.create_store('Store 2')

        count = await repo.bulk_update_runs(store1.id, store2.id)

        assert count == 0

    async def test_bulk_update_store_availabilities(self, repo, storage, sample_user):
        ...
        count = await repo.bulk_update_store_availabilities(old_store.id, new_store.id)
    ...
    async def test_count_store_runs(self, repo, storage, sample_user):
        ...
        count = await repo.count_store_runs(store.id)
...
class TestOpeningHours:
    ...
    async def test_store_with_opening_hours(self, repo, storage, sample_user):
    ...
    async def test_update_opening_hours(self, repo, storage, sample_user):
    ...
    async def test_store_without_opening_hours(self, repo):
        """Test store without opening hours (None)."""
        store = await repo.create_store('Test Store')

        assert store.opening_hours is None

    async def test_empty_opening_hours(self, repo, storage, sample_user):
...
class TestEdgeCases:
    ...
    async def test_very_long_store_name(self, repo):
        """Test with very long store name."""
        long_name = 'a' * 1000
        store = await repo.create_store(long_name)
    ...
    async def test_special_characters_in_name(self, repo):
        ...
        store = await repo.create_store(special_name)

        assert store.name == special_name
        retrieved = await repo.get_store_by_id(store.id)
        assert retrieved.name == special_name

    async def test_unicode_characters_in_address(self, repo, storage, sample_user):
    ...
    async def test_concurrent_operations(self, repo):
        ...
        all_stores = await repo.get_all_stores()
    ...
    async def test_search_with_special_characters(self, repo):
        ...
        store = await repo.create_store("Sam's Club (Downtown)")

        results = await repo.search_stores("Sam's")
...
class TestDataIntegrity:
    ...
    async def test_store_object_has_expected_fields(self, repo, sample_store_data):
    ...
    async def test_store_object_is_not_none(self, repo, sample_store_data):
    ...
    async def test_multiple_repositories_share_storage(self, storage):
        ...
        store = await repo1.create_store('Store')

        # Both repositories should see the same store
        assert await repo2.get_store_by_id(store.id) is not None
        assert len(await repo2.get_all_stores()) == 1
