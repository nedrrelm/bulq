...
class TestInMemorySessionStoreBasicOperations:
    ...
    def test_set_stores_session_data(self, store):
        ...
        result = store.get(session_token)
        assert result is not None

    def test_get_retrieves_stored_data(self, store):
    ...
    def test_get_returns_none_for_non_existent_key(self, store):
        """Test that get() returns None for non-existent key."""
        result = store.get('non-existent-token')
    ...
    def test_delete_returns_true_when_successful(self, store):
        ...
        store.set(session_token, session_data, ttl_seconds)
        result = store.delete(session_token)

        assert result is True

    def test_delete_returns_false_for_non_existent_key(self, store):
        """Test that delete() returns False for non-existent key."""
        result = store.delete('non-existent-token')

        assert result is False

    def test_session_contains_expires_at(self, store):
...
class TestInMemorySessionStoreTTL:
    ...
    def test_session_with_ttl_is_stored(self, store):
    ...
    def test_session_can_be_retrieved_before_ttl_expires(self, store):
    ...
    def test_session_expires_at_is_calculated_correctly(self, store):
    ...
    def test_session_with_different_ttl_values(self, store, ttl_seconds):
    ...
    def test_expired_session_is_removed_from_store(self, store):
    ...
    def test_zero_ttl_session(self, store):
    ...
    def test_negative_ttl_session(self, store):
...
class TestInMemorySessionStoreDataIntegrity:
    ...
    def test_storing_dict_data(self, store):
    ...
    def test_storing_complex_nested_dicts(self, store):
    ...
    def test_storing_empty_dict(self, store):
    ...
    def test_overwriting_existing_key(self, store):
        ...
        store.set(session_token, session_data_1, ttl_seconds)
        store.set(session_token, session_data_2, ttl_seconds)

        result = store.get(session_token)
        assert result['user_id'] == 'user-789'
        assert result['version'] == 2

    def test_storing_string_values(self, store):
    ...
    def test_storing_integer_values(self, store):
    ...
    def test_storing_list_values(self, store):
    ...
    def test_storing_boolean_values(self, store):
    ...
    def test_storing_none_value(self, store):
    ...
    def test_storing_uuid_in_data(self, store):
    ...
    def test_data_is_not_corrupted_after_retrieval(self, store):
        ...
        # Get multiple times
        result1 = store.get(session_token)
        result2 = store.get(session_token)
...
class TestInMemorySessionStoreEdgeCases:
    ...
    def test_unicode_in_key(self, store):
    ...
    def test_session_isolation(self, store):
        ...
        store.set('token-1', {'user_id': 'user-1'}, 3600)
        store.set('token-2', {'user_id': 'user-2'}, 3600)
        store.set('token-3', {'user_id': 'user-3'}, 3600)

        # Delete one session
        store.delete('token-2')

        # Other sessions should still exist
        assert store.get('token-1') is not None
        assert store.get('token-2') is None
        assert store.get('token-3') is not None

    def test_delete_non_existent_multiple_times(self, store):
        """Test deleting non-existent session multiple times."""
        result1 = store.delete('non-existent')
        result2 = store.delete('non-existent')
        result3 = store.delete('non-existent')

        assert result1 is False
        assert result2 is False
        assert result3 is False

    def test_get_after_delete(self, store):
        """Test getting session after deletion."""
        session_token = 'test-token-123'
        session_data = {'user_id': 'user-456'}

        store.set(session_token, session_data, 3600)
        assert store.get(session_token) is not None

        store.delete(session_token)
        assert store.get(session_token) is None
    ...
    def test_multiple_stores_are_independent(self):
        ...
        store1.set('token-1', {'user_id': 'user-1'}, 3600)
        store2.set('token-2', {'user_id': 'user-2'}, 3600)

        # Each store should only have its own session
        assert store1.get('token-1') is not None
        assert store1.get('token-2') is None

        assert store2.get('token-1') is None
        assert store2.get('token-2') is not None
...
class TestRedisSessionStore:
    ...
    def test_redis_set_stores_data(self):
        ...
        with patch.dict('sys.modules', {'redis': mock_redis_module}):
            ...
            store.set('test-token', session_data, 3600)

            mock_redis_client.hset.assert_called_once()
            mock_redis_client.expire.assert_called_once_with('session:test-token', 3600)

    def test_redis_set_serializes_datetime(self):
    ...
    def test_redis_get_returns_data(self):
        ...
        with patch.dict('sys.modules', {'redis': mock_redis_module}):
            ...
            result = store.get('test-token')

            assert result is not None
            assert result['user_id'] == 'user-456'
            mock_redis_client.hgetall.assert_called_once_with('session:test-token')

    def test_redis_get_returns_none_for_empty_data(self):
    ...
    def test_redis_get_deserializes_datetime(self):
    ...
    def test_redis_delete_removes_session(self):
        ...
        with patch.dict('sys.modules', {'redis': mock_redis_module}):
            ...
            result = store.delete('test-token')

            assert result is True
            mock_redis_client.delete.assert_called_once_with('session:test-token')

    def test_redis_delete_returns_false_for_non_existent(self):
    ...
    def test_redis_delete_handles_exception(self):
...
class TestSessionStoreFactory:
    ...
    def test_get_session_store_returns_memory_by_default(self):
    ...
    def test_get_session_store_returns_redis_when_configured(self):
    ...
    def test_get_session_store_falls_back_to_memory_when_redis_url_missing(self):
    ...
    def test_get_session_store_falls_back_to_memory_on_redis_error(self):
    ...
    def test_init_session_store_creates_global_instance(self):
    ...
    def test_get_store_initializes_if_none(self):
        """Test that get_store initializes store if not exists."""
        with (
            patch('app.infrastructure.config.SESSION_STORE_TYPE', 'memory'),
            patch('app.infrastructure.session_store._session_store', None),
        ):
