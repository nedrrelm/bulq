...
class TestRemoveMember:
    ...
    async def test_remove_member_success(self, test_user, test_group_member):
        ...
        service.group_repo.is_user_group_admin = AsyncMock(
        ...
        service.group_repo.remove_group_member = AsyncMock(return_value=True)
    ...
    async def test_remove_member_not_admin(self, test_user, test_group_member):
        ...
        service = GroupMembershipService(mock_db)
        service.group_repo.get_group_by_id = AsyncMock(return_value=mock_group)
        service.group_repo.is_user_group_admin = AsyncMock(return_value=False)  # Not admin
    ...
    async def test_remove_member_cannot_remove_admin(self, test_user, test_group_member):
        ...
        service = GroupMembershipService(mock_db)
        service.group_repo.get_group_by_id = AsyncMock(return_value=mock_group)
        service.group_repo.is_user_group_admin = AsyncMock(return_value=True)  # Both are admins
    ...
    async def test_remove_member_invalid_group_id(self, test_user, test_group_member):
    ...
    async def test_remove_member_invalid_member_id(self, test_user):
    ...
    async def test_remove_member_group_not_found(self, test_user, test_group_member):
    ...
    async def test_remove_member_not_a_member(self, test_user, test_group_member):
        ...
        service = GroupMembershipService(mock_db)
        service.group_repo.get_group_by_id = AsyncMock(return_value=mock_group)
        service.group_repo.is_user_group_admin = AsyncMock(side_effect=[True, False])
        service.group_repo.get_group_members_with_admin_status = AsyncMock(
            return_value=[
                {'id': str(test_user.id), 'name': 'Test User', 'is_group_admin': True},
            ]
    ...
    async def test_remove_member_with_active_runs(self, test_user, test_group_member):
        ...
        service.bid_repo.get_bids_by_participation = AsyncMock(return_value=[])
        service.user_repo.get_user_by_id = AsyncMock(return_value=test_group_member)
...
class TestLeaveGroup:
    ...
    async def test_leave_group_success(self, test_user, test_group_member):
        ...
        with (
            ...
            # Act
            result = await service.leave_group(group_id=str(group_id), user=test_group_member)
    ...
    async def test_leave_group_last_admin_cannot_leave(self, test_user):
        ...
        # Act & Assert
        with pytest.raises(ForbiddenError) as exc_info:
            await service.leave_group(group_id=str(group_id), user=test_user)

        assert exc_info.value.code == LAST_ADMIN_CANNOT_LEAVE
        assert 'only admin' in exc_info.value.message.lower()

    async def test_leave_group_admin_can_leave_when_other_admins_exist(
        ...
        with (
            ...
            # Act
            result = await service.leave_group(group_id=str(group_id), user=test_user)

            # Assert
            assert result is not None
            assert result.code == GROUP_LEFT

    async def test_leave_group_not_a_member(self, test_user):
    ...
    async def test_leave_group_invalid_group_id(self, test_user):
        ...
        # Act & Assert
        with pytest.raises(BadRequestError) as exc_info:
            await service.leave_group(group_id='invalid-uuid', user=test_user)

        assert exc_info.value.code == INVALID_UUID_FORMAT

    async def test_leave_group_not_found(self, test_user):
    ...
    async def test_leave_group_cancels_led_runs(self, test_user):
...
class TestPromoteMemberToAdmin:
    ...
    async def test_promote_member_success(self, test_user, test_group_member):
        ...
        service.group_repo.set_group_member_admin = AsyncMock(return_value=True)
    ...
    async def test_promote_member_not_admin(self, test_user, test_group_member):
    ...
    async def test_promote_member_already_admin(self, test_user, test_group_member):
    ...
    async def test_promote_member_not_a_member(self, test_user, test_group_member):
    ...
    async def test_promote_member_invalid_group_id(self, test_user, test_group_member):
    ...
    async def test_promote_member_invalid_member_id(self, test_user):
    ...
    async def test_promote_member_group_not_found(self, test_user, test_group_member):
    ...
    async def test_promote_member_promotion_fails(self, test_user, test_group_member):
        ...
        service.group_repo.set_group_member_admin = AsyncMock(return_value=False)
    ...
    async def test_promote_member_multiple_admins(self, test_user):
...
class TestRemoveMemberWithRuns:
    ...
    async def test_remove_member_deletes_bids_from_active_run(self, test_user, test_group_member):
        ...
        service.run_repo.get_runs_by_group = AsyncMock(return_value=[mock_run])
        service.run_repo.get_run_participations = AsyncMock(return_value=[mock_participation])
        service.bid_repo.get_bids_by_participation = AsyncMock(return_value=[mock_bid])
    ...
    async def test_remove_member_does_not_delete_bids_from_completed_run(
        self, test_user, test_group_member
    ):
    ...
    async def test_remove_member_cancels_run_if_leader(self, test_user, test_group_member):
    ...
    async def test_remove_member_does_not_cancel_completed_run_even_if_leader(
        self, test_user, test_group_member
    ):
...
class TestEdgeCases:
    ...
    async def test_remove_member_repository_failure(self, test_user, test_group_member):
        ...
        service.group_repo.remove_group_member = AsyncMock(return_value=False)
    ...
    async def test_concurrent_admin_removal_prevention(self, test_user, test_group_member):
    ...
    async def test_promote_self_to_admin(self, test_user):
        ...
        service = GroupMembershipService(mock_db)
        service.group_repo.get_group_by_id = AsyncMock(return_value=mock_group)
        service.group_repo.is_user_group_admin = AsyncMock(return_value=True)  # Already admin
        service.group_repo.get_group_members_with_admin_status = AsyncMock(
            return_value=[
                {'id': str(test_user.id), 'name': 'Test User', 'is_group_admin': True},
            ]
    ...
    async def test_leave_group_with_multiple_cancelled_runs(self, test_user):
        ...
        service.group_repo.is_user_group_admin = AsyncMock(return_value=False)
        service.group_repo.remove_group_member = AsyncMock(return_value=True)
        service.run_repo.get_runs_by_group = AsyncMock(return_value=[r[0] for r in runs])
        service.run_repo.get_run_participations = AsyncMock(side_effect=[[r[1]] for r in runs])
        service.bid_repo.get_bids_by_participation = AsyncMock(return_value=[])
        service.user_repo.get_user_by_id = AsyncMock(return_value=test_user)
