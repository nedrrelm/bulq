...
def create_group(db_session, create_user):
    ...
    def _create_group(name='Test Group', creator=None):
        if creator is None:
            creator = create_user()
        group = Group(
            name=name,
...
def create_store(db_session, create_user):
    ...
    def _create_store(name='Test Store', creator=None):
        if creator is None:
            creator = create_user()
...
def create_product(db_session, create_user):
    ...
    def _create_product(name='Test Product', creator=None):
        if creator is None:
            creator = create_user()
...
def create_run(db_session, create_group, create_store, create_user):
    ...
    def _create_run(group=None, store=None, leader=None, state=RunState.PLANNING, comment=None):
        if leader is None:
            leader = create_user()
        if group is None:
            group = create_group(creator=leader)
        if store is None:
            store = create_store(creator=leader)
        run = Run(
            group_id=group.id,
            store_id=store.id,
            state=state,
...
def create_participation(db_session):
    ...
    def _create_participation(user, run, is_leader=False, is_helper=False):
        participation = RunParticipation(
            user_id=user.id,
            run_id=run.id,
            is_leader=is_leader,
