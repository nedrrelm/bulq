...
def get_sale_repository(db: AsyncSession = None):
    """Get sale repository based on configuration mode."""
    if REPO_MODE == 'memory':
        return MemorySaleRepository(_get_memory_storage())
    else:
        _validate_database_session(db)
        return DatabaseSaleRepository(db)
