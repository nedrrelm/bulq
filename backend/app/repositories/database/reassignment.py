...
class DatabaseReassignmentRepository(AbstractReassignmentRepository):
    ...
    async def get_reassignment_request_by_id(
        self, request_id: UUID
    ) -> LeaderReassignmentRequest | None:
        """Get a reassignment request by ID."""
        result = await self.db.execute(
            select(LeaderReassignmentRequest).where(LeaderReassignmentRequest.id == request_id)
        )
        return result.scalar_one_or_none()

    async def get_pending_reassignment_for_run(
        self, run_id: UUID
    ) -> LeaderReassignmentRequest | None:
        """Get pending reassignment request for a run (if any)."""
        result = await self.db.execute(
            select(LeaderReassignmentRequest).where(
    ...
    async def get_pending_reassignments_from_user(
        self, user_id: UUID
    ) -> list[LeaderReassignmentRequest]:
    ...
    async def get_pending_reassignments_to_user(
        self, user_id: UUID
    ) -> list[LeaderReassignmentRequest]:
