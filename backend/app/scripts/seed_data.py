...
TEST_PASSWORD_HASH = bcrypt.hashpw(b'password', bcrypt.gensalt()).decode('utf-8')
...
async def create_seed_data(db_session=None):
    ...
    alice = await user_repo.create_user('Alice Johnson', 'alice', TEST_PASSWORD_HASH)
    bob = await user_repo.create_user('Bob Smith', 'bob', TEST_PASSWORD_HASH)
    carol = await user_repo.create_user('Carol Davis', 'carol', TEST_PASSWORD_HASH)
    test_user = await user_repo.create_user('Test User', 'test', TEST_PASSWORD_HASH)
    test_user.is_admin = True

    # Additional users for testing merge functionality
    await user_repo.create_user('David Williams', 'david', TEST_PASSWORD_HASH)
    emily = await user_repo.create_user('Emily Brown', 'emily', TEST_PASSWORD_HASH)
    frank = await user_repo.create_user('Frank Miller', 'frank', TEST_PASSWORD_HASH)
    ...
    await group_repo.add_group_member(
        friends_group.id, frank
    )  # Regular member with run participation
    ...
    almond_butter = await product_repo.create_product(
        'Kirkland Almond Butter', brand='Kirkland', unit='kg'
    )
    frozen_berries = await product_repo.create_product(
        'Organic Frozen Berry Mix', brand='Organic', unit='kg'
    )
    toilet_paper = await product_repo.create_product(
        'Charmin Ultra Soft 24-pack', brand='Charmin', unit='pack'
    )
    coffee_beans = await product_repo.create_product(
        'Kirkland Colombian Coffee', brand='Kirkland', unit='kg'
    )
    laundry_pods = await product_repo.create_product(
        'Tide Pods 81-count', brand='Tide', unit='pack'
    )
    ...
    availability = await product_repo.create_product_availability(
    ...
    availability = await product_repo.create_product_availability(
        almond_butter.id, costco.id, 9.99, ''
    )
    ...
    availability = await product_repo.create_product_availability(
        detergent.id, sams.id, 15.98, 'on sale'
    )
    # days_ago=0, no backdating needed

    availability = await product_repo.create_product_availability(
        laundry_pods.id, sams.id, 18.98, ''
    )
    availability.created_at = datetime.now(UTC) - timedelta(days=2)
    availability.updated_at = datetime.now(UTC) - timedelta(days=2)

    availability = await product_repo.create_product_availability(
        ground_beef.id, sams.id, 16.48, ''
    )
    availability.created_at = datetime.now(UTC) - timedelta(days=2)
    availability.updated_at = datetime.now(UTC) - timedelta(days=2)
    ...
    run_active = await run_repo.create_run(
        friends_group.id, sams.id, test_user.id, leader_fee=200.00
    )
    ...
    alice_confirmed_p = await run_repo.create_participation(
        alice.id, run_confirmed.id, is_leader=False
    )
    ...
    carol_confirmed_p = await run_repo.create_participation(
        carol.id, run_confirmed.id, is_leader=False
    )
    ...
    await bid_repo.create_or_update_bid(
        carol_confirmed_p.id, paper_towels.id, 0, True
    )  # interested only
    ...
    alice_shopping_p = await run_repo.create_participation(
        alice.id, run_shopping.id, is_leader=False
    )
    ...
    shopping_item2 = await shopping_repo.create_shopping_list_item(
        run_shopping.id, laundry_pods.id, 1
    )
    shopping_item2.is_purchased = False
    shopping_item2.purchase_order = 2

    shopping_item3 = await shopping_repo.create_shopping_list_item(
        run_shopping.id, ground_beef.id, 2
    )
    ...
    alice_adjusting_p = await run_repo.create_participation(
        alice.id, run_adjusting.id, is_leader=False
    )
    ...
    carol_adjusting_p = await run_repo.create_participation(
        carol.id, run_adjusting.id, is_leader=False
    )
    ...
    await bid_repo.create_or_update_bid(test_adjusting_p.id, rotisserie_chicken.id, 2, False)
    await bid_repo.create_or_update_bid(carol_adjusting_p.id, rotisserie_chicken.id, 1, False)

    # Paper Towels - NOT purchased (decided not to buy)
    await bid_repo.create_or_update_bid(alice_adjusting_p.id, paper_towels.id, 1, False)
    await bid_repo.create_or_update_bid(bob_adjusting_p.id, paper_towels.id, 1, False)

    # Olive Oil - CORRECTLY purchased (requested 3, purchased 3) - no adjustment needed
    await bid_repo.create_or_update_bid(test_adjusting_p.id, olive_oil.id, 1, False)
    await bid_repo.create_or_update_bid(alice_adjusting_p.id, olive_oil.id, 1, False)
    await bid_repo.create_or_update_bid(carol_adjusting_p.id, olive_oil.id, 1, False)

    # Quinoa - CORRECTLY purchased (requested 2, purchased 2) - no adjustment needed
    await bid_repo.create_or_update_bid(bob_adjusting_p.id, quinoa.id, 1, False)
    await bid_repo.create_or_update_bid(carol_adjusting_p.id, quinoa.id, 1, False)
    ...
    # Rotisserie Chicken - NOT purchased (out of stock)
    shopping_item_chicken = await shopping_repo.create_shopping_list_item(
    ...
    # Paper Towels - NOT purchased (decided not to buy)
    shopping_item_pt_adj = await shopping_repo.create_shopping_list_item(
    ...
    # Olive Oil - CORRECTLY purchased (requested 3, purchased 3) - no adjustment needed
    shopping_item_oil = await shopping_repo.create_shopping_list_item(
    ...
    # Quinoa - CORRECTLY purchased (requested 2, purchased 2) - no adjustment needed
    shopping_item_quinoa_adj = await shopping_repo.create_shopping_list_item(
    ...
    alice_distributing_p = await run_repo.create_participation(
        alice.id, run_distributing.id, is_leader=False
    )
    alice_distributing_p.is_ready = True
    bob_distributing_p = await run_repo.create_participation(
        bob.id, run_distributing.id, is_leader=False
    )
    bob_distributing_p.is_ready = True

    # Bids with distributed quantities
    bid6 = await bid_repo.create_or_update_bid(
        test_distributing_p.id, rotisserie_chicken.id, 2, False
    )
    bid6.distributed_quantity = 2
    bid6.distributed_price_per_unit = 4.99

    bid7 = await bid_repo.create_or_update_bid(
        alice_distributing_p.id, rotisserie_chicken.id, 1, False
    )
    ...
    await bid_repo.create_or_update_bid(alice_distributing_p.id, almond_butter.id, 1, False)
    await bid_repo.create_or_update_bid(bob_distributing_p.id, almond_butter.id, 1, False)

    # Frozen Berries - NOT purchased (decided not to buy)
    await bid_repo.create_or_update_bid(test_distributing_p.id, frozen_berries.id, 2, False)
    ...
    shopping_item7 = await shopping_repo.create_shopping_list_item(
    ...
    shopping_item8 = await shopping_repo.create_shopping_list_item(
    ...
    shopping_item_almond_dist = await shopping_repo.create_shopping_list_item(
    ...
    shopping_item_berries_dist = await shopping_repo.create_shopping_list_item(
    ...
    alice_completed_p = await run_repo.create_participation(
        alice.id, run_completed.id, is_leader=False
    )
    ...
    # Shopping list items
    shopping_item9 = await shopping_repo.create_shopping_list_item(
        run_completed.id, detergent.id, 1
    )
    ...
    shopping_item10 = await shopping_repo.create_shopping_list_item(
        run_completed.id, laundry_pods.id, 1
    )
    ...
    bob_completed_2_p = await run_repo.create_participation(
        bob.id, run_completed_2.id, is_leader=False
    )
    ...
    frank_completed_2_p = await run_repo.create_participation(
    ...
    shopping_item11 = await shopping_repo.create_shopping_list_item(
        run_completed_2.id, olive_oil.id, 1
    )
    ...
    shopping_item12 = await shopping_repo.create_shopping_list_item(
    ...
    shopping_item_quinoa = await shopping_repo.create_shopping_list_item(
        run_completed_2.id, quinoa.id, 1
    )
    ...
    carol_completed_3_p = await run_repo.create_participation(
    ...
    shopping_item13 = await shopping_repo.create_shopping_list_item(
        run_completed_3.id, detergent.id, 2
    )
    ...
    shopping_item14 = await shopping_repo.create_shopping_list_item(
        run_completed_4.id, olive_oil.id, 1
    )
    ...
    carol_completed_5_p = await run_repo.create_participation(
    ...
    shopping_item15 = await shopping_repo.create_shopping_list_item(
        run_completed_5.id, ground_beef.id, 1
    )
