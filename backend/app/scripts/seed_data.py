...
from models import Group, Product, ProductBid, Run, Store, User
    ...
    try:
        ...
        products_data = [
            # Costco products
            {
                ...
                'name': 'Kirkland Olive Oil (3L)',
                'base_price': Decimal('24.99'),
            },
            {
                'store_id': stores[0].id,
                'name': 'Organic Quinoa (4.5kg)',
            ...
            # Sam's Club products
            {
                ...
                'name': "Member's Mark Detergent (5.5kg)",
                'base_price': Decimal('16.98'),
            },
            {
                'store_id': stores[1].id,
                'name': 'Fresh Salmon Fillet (2kg)',
                'base_price': Decimal('32.98'),
            },
            {'store_id': stores[1].id, 'name': 'Bulk Rice (9kg)', 'base_price': Decimal('14.98')},
            # BJ's products
            {
                'store_id': stores[2].id,
                'name': 'Wellsley Farms Coffee (1.4kg)',
                'base_price': Decimal('12.99'),
            },
            {
                'store_id': stores[2].id,
                'name': 'Fresh Berries Mix (1kg)',
        ...
        # Create Runs
        runs_data = [
            {'group_id': groups[0].id, 'store_id': stores[0].id, 'state': 'active'},
            {'group_id': groups[1].id, 'store_id': stores[1].id, 'state': 'planning'},
            {'group_id': groups[2].id, 'store_id': stores[0].id, 'state': 'confirmed'},
        ]
