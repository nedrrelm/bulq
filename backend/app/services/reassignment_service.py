...
class ReassignmentService(BaseService):
    ...
    async def _create_reassignment_record(
        ...
        return await self.reassignment_repo.create_reassignment_request(
            run_id, from_user_id, to_user_id
        )
    ...
    async def accept_reassignment(
        ...
        """
        request = await self._validate_pending_request(request_id, accepting_user, 'accept')
        ...
        if self.db:
            async with transaction(self.db, 'accept leader reassignment'):
                await self._transfer_leadership(
                    request.run_id, request.from_user_id, request.to_user_id
                )
    ...
    async def _validate_pending_request(
    ...
    async def _transfer_leadership(
        self, run_id: UUID, from_user_id: UUID, to_user_id: UUID
    ) -> None:
    ...
    async def decline_reassignment(
        ...
        Raises:
            ...
            ValidationError: Request is not pending
        """
        request = await self._validate_pending_request(request_id, declining_user, 'decline')
        run = await self._get_run(request.run_id)
        store_name = await self._get_store_name(run.store_id)
    ...
    async def cancel_reassignment(
        self, request_id: UUID, cancelling_user: User
    ) -> ReassignmentResponse:
    ...
    async def _format_request(
        self, request: LeaderReassignmentRequest
        ...
        # Get store name
        store_name = await self._get_store_name(run.store_id) if run else 'Unknown Store'
